...
LedClocker::clearTime() {
	if ( reversed ) {
		strip->setPixelColor((60-seconds)%60,0);
		strip->setPixelColor((60-minutes)%60,0);
		strip->setPixelColor((60-((hours*5)+(minutes/12)))%60,0);
	} else {
		strip->setPixelColor(seconds,0);
		strip->setPixelColor(minutes,0);
		strip->setPixelColor((hours*5)+(minutes/12),0);
	}
}
